<?php
  ...
  $coverFile = $post->getCoverSmall() !== null && file_exists(ROOT . 'usercontent/blog/' . $post->getCover())
  ? h($post->getCover())
  : 'no-photo@2x.jpg';
  $coverPath = HOST . 'usercontent/blog/';

  $title = $post->getTitle() !== null ? h(shortText($post->getTitle(), $limit = 200)) : 'Статья блога "vvintage"';
  $description = h(shortText($post->getDesc(), $limit = 50));
  $views = $post->getViews() !== null ?  h($post->getViews()) : '0';

  $date = $post->getTime() !== null ? h($post->getTime()) : time();
